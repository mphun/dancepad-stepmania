...
#define NEO_PIN 0
#define N_LEDS  159 // TOTAL number of LEDs in strip
#include <Adafruit_NeoPixel.h>
...
int pinNumbers[NUM_INPUTS] = {
  ...
  2, 
  1,
 // 0,        // left side of female header, KEBYBOARD
  ...
  20, 
  19, 
  18   // right side of female header, MOUSE
};
...
void ledStripOn(int pos, int output) {
  uint32_t c;
  c = strip.Color(0, 0, 0);
  switch (pos){
    case 6: //front left pad
      if (output)
      {
      ...
      break;
    case 7:  //front right pad
      if (output)
      {
        c = strip.Color(255, 0, 0);
      }
      for(uint16_t i=(strip.numPixels() - 20); i< strip.numPixels(); i++) 
      {
        strip.setPixelColor(i, c);
      }
      break;
